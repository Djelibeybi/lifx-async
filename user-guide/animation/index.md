...
This guide covers how to use the animation module for high-frequency frame delivery to LIFX devices. The animation system is designed for real-time effects and applications that need to push color data at 30+ FPS.
...
1. Positions are in "tile-width units" (1.0 = one tile width)
1. A canvas is created that encompasses all tiles
1. Your input frame is interpreted as a 2D row-major image
1. Each tile extracts its region from the canvas based on its position
...
For matrix devices with the `has_chain` capability (like the original LIFX Tile), tiles may be physically rotated. The animator automatically handles orientation correction:
...
For a complete example including vectorized RGB to HSBK conversion, see [examples/16_animation_numpy.py](https://github.com/Djelibeybi/lifx-async/blob/main/examples/16_animation_numpy.py).
...
1. Ensure good WiFi signal to the device
1. Consider wired connection if possible
1. Accept that some packet loss is normal for UDP
...
1. For other matrix devices (Ceiling, Candle, Path), canvas equals tile size
...
1. Check tile accelerometer data via `device.device_chain`
1. Physical tiles must be stable (not moving) for accurate orientation
