# Requirements Archive: v1.2 Theme Library Update
...
- [x] **THEME-01**: Caller can get any of the 139 non-sport app themes from `ThemeLibrary`
      by ASCII slug.
      **Amended 2026-08-27** (v1.2 milestone audit): 138 ship, not 139. The capture holds
      139 non-sport records, but one slug collision survived the sport exclusion, so the
      139 captured records resolve to 138 `lifx-app` slugs. Docs and the catalogue drift
      test agree at 138
...
- [x] **FIDELITY-01**: The 25 shipped non-sport themes selected by
      ...
      sport category.
      **Amended 2026-08-27** (v1.2 milestone audit): satisfied by the *first* branch, not
      the second. Phase 8 could only reach "no device-based method can supply it", because
      a device readback clips every palette at `MAX_PALETTE_COLORS`. The Phase 9 resync
      (`291e7e6`) then obtained the true palettes for exactly those 25 themes from an
      internal LIFX HTTP API endpoint, a non-device method, so their true lengths (18 to
      68 colours) now ship in `data/themes.jsonl`. The selection clause above therefore
      no longer selects anything in the current data; it describes the pre-resync blob.
      Phase 8's record is restamped `status: historical` and pinned to
      `data/themes.jsonl@291e7e6~1`; the live guard is
      `tests/test_theme/test_ceiling_supersession.py`
...
see `.planning/ROADMAP.md` Phase Details. 16/16 live requirements mapped, no orphans;
