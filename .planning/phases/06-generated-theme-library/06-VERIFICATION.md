...
| 13 | ~~`Theme.__eq__` palette-only multiset, non-Theme returns NotImplemented, Theme unhashable (D-19/20)~~ **superseded pre-release**: `Theme.palette_equals()` palette-only multiset, `TypeError` on a non-Theme, `Theme` hashable with identity `==` (D-19a/20a) | ✓ VERIFIED (re-verified after PR #196 review item 15, commit 9b2a751) | `hash(theme)` works; `theme == theme` only for the same object; `palette_equals()` equal across consecutive gets; `palette_equals(3)` raises TypeError; pinned by TestThemePaletteEquals |
