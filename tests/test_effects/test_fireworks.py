...
class TestColorHelpers:
    """Tests for _hsb_to_rgb and _rgb_to_hsb conversion functions."""

    def test_hsb_to_rgb_red(self) -> None:
        """Test pure red conversion."""
        r, g, b = _hsb_to_rgb(0.0, 1.0, 1.0)
        assert abs(r - 1.0) < 0.01
        assert abs(g) < 0.01
        assert abs(b) < 0.01

    def test_hsb_to_rgb_green(self) -> None:
        """Test pure green conversion."""
        r, g, b = _hsb_to_rgb(120.0, 1.0, 1.0)
        assert abs(r) < 0.01
        assert abs(g - 1.0) < 0.01
        assert abs(b) < 0.01

    def test_hsb_to_rgb_blue(self) -> None:
        """Test pure blue conversion."""
        r, g, b = _hsb_to_rgb(240.0, 1.0, 1.0)
        assert abs(r) < 0.01
        assert abs(g) < 0.01
        assert abs(b - 1.0) < 0.01

    def test_hsb_to_rgb_white(self) -> None:
        """Test white (zero saturation) conversion."""
        r, g, b = _hsb_to_rgb(0.0, 0.0, 1.0)
        assert abs(r - 1.0) < 0.01
        assert abs(g - 1.0) < 0.01
        assert abs(b - 1.0) < 0.01

    def test_hsb_to_rgb_black(self) -> None:
        """Test black (zero brightness) conversion."""
        r, g, b = _hsb_to_rgb(0.0, 1.0, 0.0)
        assert abs(r) < 0.01
        assert abs(g) < 0.01
        assert abs(b) < 0.01

    def test_hsb_to_rgb_yellow(self) -> None:
        """Test yellow (hue=60) conversion."""
        r, g, b = _hsb_to_rgb(60.0, 1.0, 1.0)
        assert abs(r - 1.0) < 0.01
        assert abs(g - 1.0) < 0.01
        assert abs(b) < 0.01

    def test_hsb_to_rgb_cyan(self) -> None:
        """Test cyan (hue=180) conversion."""
        r, g, b = _hsb_to_rgb(180.0, 1.0, 1.0)
        assert abs(r) < 0.01
        assert abs(g - 1.0) < 0.01
        assert abs(b - 1.0) < 0.01

    def test_hsb_to_rgb_magenta(self) -> None:
        """Test magenta (hue=300) conversion."""
        r, g, b = _hsb_to_rgb(300.0, 1.0, 1.0)
        assert abs(r - 1.0) < 0.01
        assert abs(g) < 0.01
        assert abs(b - 1.0) < 0.01

    def test_rgb_to_hsb_red(self) -> None:
        """Test RGB red to HSB conversion."""
        h, s, b = _rgb_to_hsb(1.0, 0.0, 0.0)
        assert abs(h) < 1.0 or abs(h - 360.0) < 1.0
        assert abs(s - 1.0) < 0.01
        assert abs(b - 1.0) < 0.01

    def test_rgb_to_hsb_green(self) -> None:
        """Test RGB green to HSB conversion."""
        h, s, b = _rgb_to_hsb(0.0, 1.0, 0.0)
        assert abs(h - 120.0) < 1.0
        assert abs(s - 1.0) < 0.01
        assert abs(b - 1.0) < 0.01

    def test_rgb_to_hsb_black(self) -> None:
        """Test RGB black to HSB conversion."""
        h, s, b = _rgb_to_hsb(0.0, 0.0, 0.0)
        assert abs(b) < 0.01

    def test_rgb_to_hsb_white(self) -> None:
        """Test RGB white to HSB conversion."""
        h, s, b = _rgb_to_hsb(1.0, 1.0, 1.0)
        assert abs(s) < 0.01
        assert abs(b - 1.0) < 0.01
    ...
    def test_roundtrip_hsb_rgb_hsb(self) -> None:
        ...
        for h_deg in [0.0, 60.0, 120.0, 180.0, 240.0, 300.0]:
            r, g, b = _hsb_to_rgb(h_deg, 0.8, 0.7)
            h2, s2, b2 = _rgb_to_hsb(r, g, b)
            assert abs(h2 - h_deg) < 1.0 or abs(abs(h2 - h_deg) - 360.0) < 1.0
            assert abs(s2 - 0.8) < 0.02
            assert abs(b2 - 0.7) < 0.02
