...
def sample_a_record_data() -> bytes:
    """Sample A record data for 203.0.113.11."""
    return bytes([203, 0, 113, 11])
...
def lifx_dns_response() -> bytes:
    ...
    # Answer 1: PTR record - _lifx._udp.local -> D073D5E00034._lifx._udp.local
    ...
    # PTR target: D073D5E00034._lifx._udp.local
    ptr_target = b"\x0cD073D5E00034"  # 12 chars
    ...
    # Answer 2: SRV record - D073D5E00034._lifx._udp.local -> D073D5E00034.local
    # Build without compression for simplicity in test
    srv_name = b"\x0cD073D5E00034\x05_lifx\x04_udp\x05local\x00"
    body += srv_name
    body += struct.pack(
        "!HHIH", 33, 0x8001, 120, 0
    )  # SRV, cache-flush, TTL, placeholder
    srv_target = b"\x0cD073D5E00034\x05local\x00"
    srv_rdata = struct.pack("!HHH", 0, 0, 56700) + srv_target
    body = body[:-2] + struct.pack("!H", len(srv_rdata)) + srv_rdata

    # Answer 3: TXT record
    txt_name = b"\x0cD073D5E00034\x05_lifx\x04_udp\x05local\x00"
    body += txt_name
    txt_strings = [
        "tm=1",
        "cm=0",
    ...
    # Additional: A record - D073D5E00034.local -> 203.0.113.11
    a_name = b"\x0cD073D5E00034\x05local\x00"
    body += a_name
    body += struct.pack("!HHIH", 1, 0x8001, 120, 4)  # A record
    body += bytes([203, 0, 113, 11])
