...
class TestMirrorLightLifecycle:
    ...
    async def test_aenter_loads_state_file(self, tmp_path: Path) -> None:
        ...
        mirror._load_state_from_file = AsyncMock()
    ...
    async def test_aexit_swallows_save_failure(
        ...
        mirror = MirrorLight(serial="d073d5000100", ip="192.168.1.100")
        mirror._state_file = str(tmp_path / "mirror.json")
        mirror._save_state_to_file = AsyncMock(side_effect=OSError("disk full"))
        parent_exit = AsyncMock()
...
class TestMirrorStateFileEdgeCases:
    ...
    async def test_save_without_state_file_is_noop(self) -> None:
        """Test that saving without a configured file does nothing."""
        mirror = _connected_mirror()
        mirror._state_file = None

        await mirror._save_state_to_file()  # must not raise

    async def test_load_without_state_file_is_noop(self) -> None:
        """Test that loading without a configured file does nothing."""
        mirror = _connected_mirror()
        mirror._state_file = None

        await mirror._load_state_from_file()  # must not raise

    async def test_load_ignores_unknown_serial(self, tmp_path: Path) -> None:
    ...
    async def test_load_accepts_colors_before_version_is_known(
        self, tmp_path: Path
    ) -> None:
    ...
    async def test_load_handles_corrupt_json(
        self, tmp_path: Path, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    async def test_save_handles_write_failure(
        ...
        with patch(
            "lifx.devices.mirror.write_state_file",
            side_effect=OSError("disk full"),
        ):
            await mirror._save_state_to_file()

        assert "Failed to save state" in caplog.text

    async def test_save_merges_with_existing_entry(self, tmp_path: Path) -> None:
...
class TestMirrorComponentCoverage:
    ...
    async def test_save_skips_absent_components(self, tmp_path: Path) -> None:
