...
Each of the six pre-v1.2 category names has a recorded fate. Two map to the app
...
Nine pre-v1.2 theme keys are recorded as deprecated, each with a replacement that
...
removing a key would be a v2.0 decision. The disposition is queryable data —
...
used. The remaining 19 pre-v1.2 orphan keys are recorded as `library-only` and stay.

## Before and After

**Before** — historical pre-v1.2 behaviour, shown for context:
...
resolve** after v1.2 — what changed is the result set, not the call's validity.
