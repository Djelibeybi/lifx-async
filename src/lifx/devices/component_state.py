...
#: Seconds past a transition's end before the device's reports are trusted
#: again. Measured on a LIFX Mirror, Get64 caught up within 150 ms of a
#: transition finishing, an instant write took as long to appear, and
#: GetPower still reported the old level straight after an acknowledged
#: SetPower.
WRITE_SETTLE_MARGIN = 0.5

T = TypeVar("T")


class Pending(Generic[T]):
    """A value a component device just wrote, until the device reports it.

    Both components live on one tile, and the firmware runs one transition per
    tile: any Set64 freezes whatever transition is still running, including in
    zones it does not touch. Get64 meanwhile reports the in-flight colours, and
    GetPower lags a SetPower the same way. A component write that rebuilt the
    tile from Get64 during the other component's fade would pin that component
    at its half-faded value, and one that trusted GetPower straight after a
    power-off would take the "light is on" path and never power back on.

    Remembering the last written value until it settles avoids both. Carrying
    the other component's *target* into the next write, with the new duration,
    lets both fade together to where they were headed. Once the transition has
    had time to finish, the device is asked again, so changes made elsewhere
    (the LIFX app, another controller) are respected.
    """
    ...
    def get(self) -> T | None:
        ...
        if self._value is None or time.monotonic() >= self._settles_at:
            self._value = None
            return None
        value = self._value
